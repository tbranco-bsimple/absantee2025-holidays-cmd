...
public class CollaboratorControllerTests : IntegrationTestBase, IClassFixture<IntegrationTestsWebApplicationFactory<Program>>
{
    ...
    public async Task GetCollaborators_ReturnsOkWithList()
    {
        ...
        var CollaboratorDTO = await PostAndDeserializeAsync<CollaboratorDTO>("api/collaborators", collaborator);
        ...
        Assert.Contains(CollaboratorDTO.Id, collaborators);
    }
    ...
    public async Task FindOverlappingHolidayPeriodsBetweenTwoCollaboratorsBetweenDates_ReturnsPeriods()
    {
        ...
        var collaborator1 = await PostAndDeserializeAsync<CollaboratorDTO>("/api/collaborators", CollaboratorHelper.GenerateRandomCollaboratorDto());

        var collaborator2 = await PostAndDeserializeAsync<CollaboratorDTO>("/api/collaborators", CollaboratorHelper.GenerateRandomCollaboratorDto());
    ...
    public async Task ListHolidayPeriodContainingDay_Returns200AndObject()
    {
        ...
        var createdHolidayPeriodDTO = await PostAndDeserializeAsync<HolidayPeriodDTO>($"api/collaborators/{CollaboratorDTO.CollabId}/holidayPlan/holidayPeriod", period);

        // Act
        var result = await GetAndDeserializeAsync<HolidayPeriodDTO>($"api/collaborators/{CollaboratorDTO.CollabId}/holidayperiods/includes-date?date=2045-02-20");
    ...
    public async Task ListHolidayPeriodLongerThan_Returns200AndObjects()
    {
        ...
        var createdHolidayPeriod1DTO = await PostAndDeserializeAsync<HolidayPeriodDTO>($"api/collaborators/{CollaboratorDTO.CollabId}/holidayPlan/holidayPeriod", period);
        var createdHolidayPeriod2DTO = await PostAndDeserializeAsync<HolidayPeriodDTO>($"api/collaborators/{CollaboratorDTO.CollabId}/holidayPlan/holidayPeriod", period2);

        // Act
        var result = await GetAndDeserializeAsync<List<HolidayPeriodDTO>>($"api/collaborators/{CollaboratorDTO.CollabId}/holidayperiods/longer-than?days=5");
    ...
    public async Task ListCollaboratorsWithHolidayPeriodsLongerThan_Returns200AndObjects()
    {
        ...
        Assert.Contains(result, c => c.Id == CollaboratorDTO.CollabId);
